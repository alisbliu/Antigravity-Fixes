...
*   **Solução (`move_all_unsupported_dbs.py`, `clean_backup_repo.py` & `delete_empty_conversations.py`):** Criamos ferramentas que varrem os bancos locais para detectar passos incompatíveis e movê-los para um diretório isolado (`unsupported_dbs`), removem essas incompatibilidades do repositório de backup remoto, e removem fisicamente conversas fantasmas vazias (0 passos) geradas por falhas de inicialização, regenerando em seguida o arquivo do índice global (`agyhub_summaries_proto.pb`) para manter a barra lateral perfeitamente limpa e estável.
